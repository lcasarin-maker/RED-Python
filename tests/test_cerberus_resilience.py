...
class TestCoderCerberusResilience(unittest.TestCase):
    ...
    def test_core_mandates_presence(self):
        ...
        for i in [1, 2, 3, 4, 5, 6, 7, 8, 9, 17]:
            self.assertIn(
                f"MANDATO S{i}:", all_content, f"Falta Mandato S{i} en los manifiestos"
            )

        # Verificar mandatos de comportamiento (B1-B11)
        for i in range(1, 12):
            self.assertIn(
                f"MANDATO B{i}:", all_content, f"Falta Mandato B{i} en los manifiestos"
            )
    ...
    def test_run_security_audit_12d_dynamic_pass(self):
        ...
        self.assertGreater(len(content), 500, "run_security_audit_12d.py too short to be real")
        self.assertTrue(
            any(keyword in content for keyword in ["audit", "DeepForensic", "APPROVED", "REJECTED"]),
            "run_security_audit_12d.py missing core audit keywords"
        )
