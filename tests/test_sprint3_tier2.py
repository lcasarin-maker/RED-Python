...
class TestPermissionAuditor:
    ...
    def test_dangerous_python_c_flagged(self, tmp_path):
        ...
        f = tmp_path / "settings.json"
        self._make_settings(
            f, ["Bash(python -c 'import os; os.system(\"rm -rf /\")'])"]
        )
        findings = audit_permission_file(f)
        assert len(findings) > 0
    ...
    def test_uses_scripts_core_utils(self):
        source = (PROJECT_ROOT / "scripts" / "audit_permissions.py").read_text(
            encoding="utf-8"
        )
...
class TestEmpiricalProofChecker:
            ...
            json.dumps(
                {
                    ...
                    "operation": "some_claim",
                    "output_log": "some_claim",
                }
            ),
            encoding="utf-8",
        )
    ...
    def test_check_proof_rejects_mismatched_claim(self, tmp_path):
        from scripts.check_empirical_proof import check_proof
        ...
        (ev / "proof.json").write_text(
            json.dumps(
                {
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "operation": "different claim",
                    "output_log": "different claim",
                }
            ),
            encoding="utf-8",
        )
    ...
    def test_check_proof_rejects_stale_evidence(self, tmp_path):
        from scripts.check_empirical_proof import check_proof

        ev = tmp_path / ".protocol" / "evidence"
        ev.mkdir(parents=True)
        (ev / "stale.json").write_text(
            json.dumps(
                {
    ...
    def test_ui_files_filters_correctly(self):
        ...
        non_ui = [
            f
            for f in result
            if not any(f.endswith(e) for e in [".html", ".js", ".jsx", ".tsx", ".css"])
        ]
        assert non_ui == []
    ...
    def test_uses_scripts_core_utils(self):
        source = (PROJECT_ROOT / "scripts" / "check_empirical_proof.py").read_text(
            encoding="utf-8"
        )
        assert "from scripts.core_utils import" in source
...
class TestChunkingValidator:
    ...
    def test_uses_scripts_core_utils(self):
        source = (PROJECT_ROOT / "scripts" / "validate_chunking.py").read_text(
            encoding="utf-8"
        )
        assert "from scripts.core_utils import" in source
        assert "setup_windows_utf8" in source
...
class TestHygieneAuditor:
    ...
    def test_uses_setup_windows_utf8(self):
        source = (PROJECT_ROOT / "scripts" / "audit_hygiene.py").read_text(
            encoding="utf-8"
        )
        assert "setup_windows_utf8" in source
