...
class TestHeadspaceAutoTrigger:
    ...
    def test_estimate_context_usage_with_file(self, tmp_path):
        ...
        trigger = HeadspaceAutoTrigger(
            historial_path=h, status_path=tmp_path / "ghost.md"
        )
        result = trigger.estimate_context_usage()
        assert result > 0
    ...
    def test_trigger_compression_returns_list(self):
        ...
        with patch(
            "scripts.trigger_context_compression.subprocess.run",
        ...
        successful = [
            a
            for a in actions
            if "recommend" not in a.lower() and "compact" not in a.lower()
        ]
        assert successful == []
    ...
    def test_uses_scripts_core_utils(self):
        source = (
            PROJECT_ROOT / "scripts" / "trigger_context_compression.py"
...
class TestCompactAutomationHelper:
    def test_run_compress_historial_failure_returns_false(self, tmp_path):
        ...
        with patch(
            "scripts.compact_automation_helper.subprocess.run", return_value=mock_fail
        ):
    ...
    def test_run_compress_historial_success_returns_true(self, tmp_path):
        ...
        with patch(
            "scripts.compact_automation_helper.subprocess.run", return_value=mock_ok
        ):
            result = helper.run_compress_historial()
    ...
    def test_auto_compact_prepare_all_fail_returns_false_values(self, tmp_path):
        ...
        with patch(
            ...
            results = helper.auto_compact_prepare()
        # compact_sentinel_cleared no usa subprocess — siempre True; excluir del check
        subprocess_bools = [
            v for k, v in results.items()
            if isinstance(v, bool) and k != "compact_sentinel_cleared"
        ]
        # Negative: todos los tasks de subprocess deben fallar → ninguno True
        succeeded = [v for v in subprocess_bools if v is True]
        assert succeeded == []
        assert len(subprocess_bools) == 4
    ...
    def test_uses_sys_executable_not_python_string(self):
        source = (PROJECT_ROOT / "scripts" / "compact_automation_helper.py").read_text(
            encoding="utf-8"
        )
