...
class TestCPILock(unittest.TestCase):
    def setUp(self):
        ...
        if self.state_file.exists():
            self.original_state = json.loads(
    ...
    def tearDown(self):
        ...
        if self.original_state is not None:
            self.state_file.write_text(
                json.dumps(self.original_state, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
    ...
    def test_cpi_lock_blocking(self):
        ...
        self.assertTrue(
            any("CPI reasoning_lock activo" in v for v in violations),
    ...
    def test_cpi_unlock_cli(self):
        ...
        self.assertFalse(
            updated_state.get("reasoning_lock", True),
            "reasoning_lock should be False after unlock",
        )
        self.assertEqual(
            updated_state.get("consecutive_failures", -1),
            0,
