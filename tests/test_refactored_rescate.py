...
class TestRefactoredRescate:
    ...
    def test_b12_operativo_lista_incertidumbre(self):
        ...
        b12_section = protocol[
            protocol.find("MANDATO B12") : protocol.find("MANDATO B13")
        ]

        # Validación: B12 tiene "Al final de cada turno"
        assert (
            "Al final de cada turno" in b12_section
        ), "B12: No es operativo (falta frecuencia de ejecución)"

        # Validación: B12 especifica QUÉ listar
        assert (
            "subsistemas" in b12_section or "verificadas" in b12_section
    ...
    def test_b14_secuencial_con_b6(self):
        ...
        assert (
            "Fase 1" in b14_section and "auditar" in b14_section
    ...
    def test_setup_validate_py_is_fast(self):
        ...
        for _ in range(3):
            start = time.time()
            subprocess.run(
                ["python", str(script)], capture_output=True, text=True, timeout=5
            )
...
class TestRefactorIntegration:
    ...
    def test_b12_b14_b15_chain(self):
        ...
        assert (
            "MANDATO B12" in protocol
            and "MANDATO B13" in protocol
            and "MANDATO B14" in protocol
            and "MANDATO B15" in protocol
