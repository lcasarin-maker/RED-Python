...
class TestEvidenceLogger:
    ...
    def test_validate_operation_rejects_stale_evidence(self, tmp_path):
        from scripts.log_evidence import EvidenceLogger

        el = EvidenceLogger(root=tmp_path)
        evidence_dir = tmp_path / ".protocol" / "evidence"
        evidence_dir.mkdir(parents=True, exist_ok=True)
        stale = evidence_dir / "20000101_000000_000000_promote_deadbeef.json"
        stale.write_text(
            json.dumps(
                {
                    ...
                    "validation_domains": {"D3": True},
                    "files_touched": [],
                    "output_log": "old proof",
                }
            ),
            encoding="utf-8",
        )
        assert el.validate_operation_approved("promote") is False

    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "log_evidence.py").read_text(
            encoding="utf-8"
        )
...
class TestTokenTracker:
    ...
    def test_uses_scripts_core_utils_import(self):
        ...
        source = (PROJECT_ROOT / "scripts" / "track_tokens.py").read_text(
            encoding="utf-8"
        )
...
class TestAlertsViewer:
    ...
    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "view_alerts.py").read_text(
            encoding="utf-8"
        )
...
class TestDeadlockResolver:
    def _build_db(
        self, db_path: Path, agent: str, status: str, age_minutes: int = 0
    ) -> None:
        conn = sqlite3.connect(str(db_path))
        cursor = conn.cursor()
        cursor.execute(
    ...
    def test_check_all_empty_db_returns_zero(self, tmp_path):
        ...
        conn.execute(
            "CREATE TABLE agent_heartbeats (id INTEGER PRIMARY KEY, timestamp DATETIME, agent_id TEXT, status TEXT)"
        )
        conn.execute(
            "CREATE TABLE alerts (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, severity TEXT, type TEXT, message TEXT, agent_id TEXT)"
        )
        conn.commit()
        conn.close()
    ...
    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "resolve_deadlocks.py").read_text(
            encoding="utf-8"
        )
        forbidden_fwd = "D:" + "/GoogleDrive"
        assert forbidden_fwd not in source
...
class TestSyncBinding:
    ...
    def test_uses_setup_windows_utf8(self):
        source = (PROJECT_ROOT / "scripts" / "sync_binding.py").read_text(
            encoding="utf-8"
        )
