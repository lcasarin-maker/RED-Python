...
def test_vc082_ghost_import_detected(tmp_path):
    ...
    assert (
        _ghost_imports(good, declared) == set()
    ), "falso positivo en imports declarados/stdlib"
...
def test_vc121_duplicate_function_names(tmp_path):
    ...
    assert (
        ok == set()
    ), f"falso positivo: nombres distintos marcados como colisión: {ok}"
...
def test_vc122_no_pip_install_in_scripts():
    ...
    ), "no cazó la llamada pip install real"
    assert not pred(
        '"Disabled automatic subprocess pip installs in repair_failing_tests.py"'
    ), "falso positivo en un string descriptivo (sin llamada)"
    assert not pred(
        "print('Ejecuta: pip install pyperclip')"
    ), "falso positivo en un print de ayuda"
    # REPO-GUARD: ningún script .py debe invocar pip install por subprocess.
    offenders = _scan_scripts(pred)
    assert (
        offenders == []
    ), f"VC-122: pip install por subprocess en scripts/: {offenders}"
...
def test_vc123_no_git_add_all_in_scripts():
    ...
    assert not pred(
        "subprocess.run(['git', 'add', 'archivo_especifico.py'])"
...
def test_tk005_status_md_has_required_sections(tmp_path):
    ...
    assert (
        _status_missing_sections(complete) == []
    ), f"falso positivo: {_status_missing_sections(complete)}"
...
def test_tk018_external_backlog_exists(tmp_path):
    ...
    assert not _has_external_backlog(
        tmp_path
    ), "detector dio falso positivo en root vacío"
    ...
    assert _has_external_backlog(
        tmp_path
    ), "no detectó el backlog externalizado presente"
    # REPO-GUARD: el repo real tiene el backlog externalizado.
    assert _has_external_backlog(
        _ROOT
    ), "TK-018: el repo no tiene .protocol/review_queue.json"
