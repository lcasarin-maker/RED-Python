...
class TestCoderCerberusMandates(unittest.TestCase):
    ...
    def test_S17_universal_version_parity(self):
        ...
        self.assertNotEqual(
            target_version, "UNKNOWN", "VERSION.txt no encontrado o vacío"
        )
    ...
    def test_S1_run_security_audit_12d_has_forensic_auditor(self):
        ...
        source = (self.root / "scripts" / "run_security_audit_12d.py").read_text(
            encoding="utf-8"
        )
        self.assertIn(
    ...
    def test_S7_no_shell_file_writes_in_scripts(self):
        ...
        self.assertEqual(
            violations, [], "S7 shell-write violations:\n" + "\n".join(violations)
        )
    ...
    def test_B7_evidence_directory_has_valid_records(self):
        ...
        self.assertGreater(
            len(json_files), 0, "B7: No hay evidencia JSON en .protocol/evidence/"
        )
        for ev_file in json_files:
            try:
                data = json.loads(ev_file.read_text(encoding="utf-8"))
                self.assertIn(
                    "timestamp", data, f"B7: {ev_file.name} no tiene campo timestamp"
                )
    ...
    def test_B10_agent_state_has_valid_handoff(self):
        ...
        self.assertEqual(
            missing, set(), f"B10: Campos faltantes en .agent_state.json: {missing}"
        )
        self.assertNotIn(
            "Gemini",
            data.get("agent_name", ""),
    ...
    def test_S15_no_zombie_protocol_folder(self):
        ...
        self.assertFalse(
            root_protocol.exists(), "Carpeta zombi .protocolo detectada en la raíz"
        )
