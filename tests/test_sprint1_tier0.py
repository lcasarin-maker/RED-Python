...
class TestCoreUtils:
    def test_no_hardcoded_root_dir(self):
        ...
        source = (PROJECT_ROOT / "scripts" / "core_utils.py").read_text(
            encoding="utf-8"
        )
        assert "ROOT_DIR" not in source, "ROOT_DIR hardcode persiste en core_utils.py"
        assert (
            "PROTOCOL_SOURCE" not in source
        ), "PROTOCOL_SOURCE hardcode persiste en core_utils.py"
    ...
    def test_setup_windows_utf8_guard_flag(self):
        ...
        assert (
            "_cerberus_utf8_wrapped" in source
    ...
    def test_setup_token_events_db_creates_table(self, tmp_path):
        ...
        conn.commit()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='token_events'"
        )
...
class TestRTKAutoCompress:
    ...
    def test_process_output_truncates_long_lines(self):
        ...
        for line in compressed.split("\n"):
            assert (
                len(line) <= RTKAutoCompress.LINE_LIMIT
            ), f"Línea demasiado larga: {len(line)}"
...
class TestPostMoveValidator:
    def test_no_moves_returns_true(self):
        from scripts.validate_post_move import validate_post_move
    ...
    def test_detects_deleted_files(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(stdout="D\told_file.py\n", returncode=0)
    ...
    def test_uses_sys_executable(self):
        ...
        source = (PROJECT_ROOT / "scripts" / "validate_post_move.py").read_text(
            encoding="utf-8"
        )
        assert (
            '"python"' not in source
        ), "Hardcode 'python' encontrado — usar sys.executable"
...
class TestRollbackTester:
    ...
    def test_no_strftime_s_windows_bug(self):
        ...
        source = (PROJECT_ROOT / "scripts" / "verify_rollback.py").read_text(
            encoding="utf-8"
        )
        assert (
            "strftime('%s')" not in source
        ), "Bug de strftime('%s') en Windows detectado"
...
class TestGuardrailStrict:
    def test_no_hardcodes(self):
        ...
        source = (PROJECT_ROOT / "scripts" / "detect_rule_code_drift.py").read_text(
            encoding="utf-8"
        )
...
class TestValidateSecurityTier:
    ...
    def test_untrusted_blocks_everything_outside_sandbox(self):
        ...
        assert (
            validate_tier_permissions("UNTRUSTED", [".agent-sandbox/untrusted/out.txt"])
            == []
        )
    ...
    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "validate_security_tier.py").read_text(
            encoding="utf-8"
        )
        forbidden_fwd = "D:" + "/GoogleDrive"
        forbidden_back = "D:" + chr(92) + "GoogleDrive"
...
class TestValidateRouting:
    ...
    def test_missing_json_block_returns_violation(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "HISTORIAL.md").write_text(
            "# Sesion\nSin JSON.\n", encoding="utf-8"
        )
        violations = vr.validate_historial_routing()
    ...
    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "validate_routing.py").read_text(
            encoding="utf-8"
        )
        forbidden_fwd = "D:" + "/GoogleDrive"
        forbidden_back = "D:" + chr(92) + "GoogleDrive"
