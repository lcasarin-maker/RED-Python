...
class TestGlobalSyncManager:
    ...
    def test_load_registry_valid_json(self, tmp_path):
        ...
        self._make_registry(
            tmp_path, [{"name": "proj", "path": "/x", "role": "PROJECT"}]
        )
        mgr = GlobalSyncManager(core_path=str(tmp_path))
    ...
    def test_sync_all_dry_run_no_writes(self, tmp_path):
        ...
        mgr = GlobalSyncManager(core_path=str(tmp_path))
        with patch.object(
            mgr, "_copy_protocol_files_impl", return_value=[], create=True
        ):
    ...
    def test_no_hardcoded_paths(self):
        source = (PROJECT_ROOT / "scripts" / "global_sync_safe.py").read_text(
            encoding="utf-8"
        )
...
class TestRunComplianceTests:
    ...
    def test_run_suite_returns_true_when_all_pass(self):
        ...
        mock_result.stderr = ""
        with patch(
            "scripts.run_compliance_tests.subprocess.run", return_value=mock_result
        ):
            result = run_suite()
    ...
    def test_run_suite_returns_false_on_failure(self):
        ...
        with patch(
            "scripts.run_compliance_tests.subprocess.run", return_value=mock_fail
        ):
            result = run_suite()
    ...
    def test_run_suite_returns_false_on_exception(self):
        ...
        with patch(
            "scripts.run_compliance_tests.subprocess.run",
    ...
    def test_no_sys_path_append(self):
        source = (PROJECT_ROOT / "scripts" / "run_compliance_tests.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_subprocess_imported_at_module_level(self):
        ...
        import_line = next(
            (i for i, l in enumerate(lines) if l.strip() == "import subprocess"), None
        )
