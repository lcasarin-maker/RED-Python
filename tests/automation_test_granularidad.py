...
def test_file_lengths():
    ...
    EXCLUDE_DOCS = {
        "MATRIZ_AUTOMATIZACION_COMPLETA.md",
        "PLAN_REMEDIACION.md",
        "HISTORIAL.md",
        "PLAN.md",
    }
                ...
                try:
                    with open(path, "r", encoding="utf-8", errors="ignore") as f:
                        ...
                        if (
                            path.endswith(".md")
                            and count > MD_LIMIT
                            and file not in EXCLUDE_DOCS
                        ):
                            print(
                                f"FALLO: Documento {path} tiene {count} lineas (Limite: {MD_LIMIT})."
                            )
                            fail_count += 1
                        elif (
                            ...
                            print(
                                f"ADVERTENCIA: Codigo {path} tiene {count} lineas. Evaluar SRP."
                            )
    ...
    assert (
        fail_count == 0
    ), f"Granularidad: {fail_count} documento(s) superan {MD_LIMIT} lineas."
