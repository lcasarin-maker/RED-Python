...
class DeadlockResolver:
    ...
    def __init__(self, db_path=".secrets/protocolo/protocol_state.db", threshold_minutes=10):
    ...
    def analyze_agent_state(self, agent_id: str) -> bool:
        ...
        if not heartbeats: return False
        statuses = [hb[1] for hb in heartbeats]
        if len(set(statuses)) == 1 and statuses[0] == 'blocked': return True

        try:
            latest_time = datetime.fromisoformat(heartbeats[0][0])
            time_since = (datetime.now() - latest_time).total_seconds()
            if time_since > self.threshold: return True
        except Exception as e:
            import logging
            logging.warning("deadlock_resolver: no se pudo parsear timestamp de %s: %s", agent_id, e)
        return False
    ...
    def insert_alert(self, agent_id: str, message: str, recommendation: str) -> None:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, ('warn', 'deadlock_detected', f"{message} | RECOMMENDATION: {recommendation}", agent_id))
        conn.commit()
        conn.close()
    ...
    def check_all_agents(self) -> int:
        ...
        for (agent_id,) in agents:
            if self.analyze_agent_state(agent_id):
                self.insert_alert(agent_id, f"Agent {agent_id} deadlocked", "Run /compact")
                alerts_generated += 1
