...
def test_setup_validation(tmp_path, monkeypatch):
    ...
    assert any("AGENT.md" in e for e in broken_stack.errors), (
    ...
    _make_healthy_stack_registry_corrupt = tmp_path / ".protocol" / "metadata" / "REGISTRY.json"
    (tmp_path / "AGENT.md").write_text("# stub\n", encoding="utf-8")  # restaura esencial
    _make_healthy_stack_registry_corrupt.write_text("{ this is not json", encoding="utf-8")
    ...
    assert any("REGISTRY.json" in e for e in broken_registry.errors), (
...
def test_pre_commit_hook_exists_and_executable(tmp_path, monkeypatch):
    ...
    assert any("pre-commit" in e for e in missing.errors), (
...
def test_infrastructure_checks(tmp_path):
    ...
    assert any("ghost_orphan_module.py" in e for e in errors), (
    ...
    (scripts_dir / "real_lib.py").write_text("def helper():\n    return 1\n", encoding="utf-8")
    ...
    assert not any("real_lib.py" in e for e in errors2), (
