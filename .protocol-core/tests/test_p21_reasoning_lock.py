...
- rigor_maestro.__main__ solo llama update_lock_state con --track-lock (default OFF),
...
from scripts import rigor_maestro


def _state(tmp_path, **kw):
    """Create a tmp .agent_state.json (+ STATUS.md) and point rigor_maestro._ROOT at it."""
...
def test_update_lock_state_increments_and_locks_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rigor_maestro, "_ROOT", tmp_path)
    sf = _state(tmp_path, consecutive_failures=2, reasoning_lock=False)
    rigor_maestro.update_lock_state(success=False)
    data = _read(sf)
...
def test_update_lock_state_resets_on_success(tmp_path, monkeypatch):
    ...
    rigor_maestro.update_lock_state(success=True)
    data = _read(sf)
...
def test_main_default_does_not_track_lock():
    """P2.1: rigor_maestro.__main__ por defecto NO llama update_lock_state — solo con
    --track-lock. Bloquea la regresión que dejaba que el post-commit deadlockee."""
    source = (_ROOT / "scripts" / "rigor_maestro.py").read_text(encoding="utf-8")
