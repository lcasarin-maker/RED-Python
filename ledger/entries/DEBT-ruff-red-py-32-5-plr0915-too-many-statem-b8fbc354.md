...
<!-- Prosa: ningun gate la lee. Migrada de tasks/done/DEBT-ruff-red-py-32-5-plr0915-too-many-statem-b8fbc354.md el 2026-09-01. -->
...
<!-- findings:start -->
- ruff: red.py:32:5: PLR0915 Too many statements (74 > 50)
<!-- findings:end -->

```json queue-job
{
  "name": "remediate_DEBT-ruff-red-py-32-5-plr0915-too-many-statem-b8fbc354",
  "command": "ruff check --select PLR0915 red.py",
  "artifact": "tasks/done/DEBT-ruff-red-py-32-5-plr0915-too-many-statem-b8fbc354.md"
}
```
...
`_run_cli` in `red.py` grew past ruff's `PLR0915` complexity threshold as branching
...
sub-steps into separate functions, bringing `_run_cli` back under the threshold
without changing its externally observed behavior.

## Regression Test

`ruff check --select PLR0915 red.py` is the regression test: it fails again the
...
```
$ ruff check --select PLR0915 red.py
All checks passed!
```
