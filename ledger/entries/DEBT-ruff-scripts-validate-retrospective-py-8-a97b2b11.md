...
close_check: {"cmd": "ruff check --select PLR0912 scripts/validate_retrospective.py", "expect": "exit_zero"}
created: 2026-08-20
---

<!-- Prosa: ningun gate la lee. Migrada de tasks/done/DEBT-ruff-scripts-validate-retrospective-py-8-a97b2b11.md el 2026-09-01. -->
...
<!-- findings:start -->
- ruff: scripts/validate_retrospective.py:84:5: PLR0912 Too many branches (16 > 12)
<!-- findings:end -->

```json queue-job
{
  "name": "remediate_DEBT-ruff-scripts-validate-retrospective-py-8-a97b2b11",
  "command": "ruff check --select PLR0912 scripts/validate_retrospective.py",
  "artifact": "tasks/done/DEBT-ruff-scripts-validate-retrospective-py-8-a97b2b11.md"
}
```
...
`validate_retrospective_schema` in `scripts/validate_retrospective.py` grew past ruff's `PLR0912` complexity threshold as branching
...
`ruff check --select PLR0912 scripts/validate_retrospective.py` is the regression test: it fails again the
...
```
$ ruff check --select PLR0912 scripts/validate_retrospective.py
All checks passed!
```
