...
<!-- Prosa: ningun gate la lee. Migrada de tasks/done/DEBT-ruff-filters-py-188-5-c901-has-only-igno-06b7ff3d.md el 2026-09-01. -->
...
<!-- findings:start -->
- ruff: filters.py:188:5: C901 `has_only_ignorable_files` is too complex (13 > 10)
<!-- findings:end -->

```json queue-job
{
  "name": "remediate_DEBT-ruff-filters-py-188-5-c901-has-only-igno-06b7ff3d",
  "command": "ruff check --select C901 filters.py",
  "artifact": "tasks/done/DEBT-ruff-filters-py-188-5-c901-has-only-igno-06b7ff3d.md"
}
```
...
`has_only_ignorable_files` in `filters.py` grew past ruff's `C901` complexity threshold as branching
...
sub-steps into separate functions, bringing `has_only_ignorable_files` back under the threshold
