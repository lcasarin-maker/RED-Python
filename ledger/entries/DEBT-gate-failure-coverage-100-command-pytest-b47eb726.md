...
<!-- Prosa: ningun gate la lee. Migrada de tasks/done/DEBT-gate-failure-coverage-100-command-pytest-b47eb726.md el 2026-09-01. -->

> **REABIERTA, pero es CANDIDATA A CIERRE BARATO.** Su prosa ya nombra un control
> negativo y muestra los dos codigos de salida, asi que probablemente cumple el
> contrato y solo le falta la forma tipada. **La marca la puso un regex y un regex no
> cierra nada**: leer prosa para decidir es el defecto de DGX-411 aplicado al acto mas
> caro del contrato. Para cerrarla: corre su `close_check` en los dos sentidos, guarda
> las dos salidas en `ledger/evidence/<id>/` y pon `evidence` y `closed_at`.
