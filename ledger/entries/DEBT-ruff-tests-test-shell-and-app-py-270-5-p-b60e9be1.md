...
<!-- Prosa: ningun gate la lee. Migrada de tasks/done/DEBT-ruff-tests-test-shell-and-app-py-270-5-p-b60e9be1.md el 2026-09-01. -->
...
<!-- findings:start -->
- ruff: tests/test_shell_and_app.py:270:5: PLR0915 Too many statements (63 > 50)
<!-- findings:end -->

```json queue-job
{
  "name": "remediate_DEBT-ruff-tests-test-shell-and-app-py-270-5-p-b60e9be1",
  "command": "ruff check --select PLR0915 tests/test_shell_and_app.py",
  "artifact": "tasks/done/DEBT-ruff-tests-test-shell-and-app-py-270-5-p-b60e9be1.md"
}
```
...
`(unnamed test fn at line 270)` in `tests/test_shell_and_app.py` grew past ruff's `PLR0915` complexity threshold as branching
...
sub-steps into separate functions, bringing `(unnamed test fn at line 270)` back under the threshold
without changing its externally observed behavior.

## Regression Test

`ruff check --select PLR0915 tests/test_shell_and_app.py` is the regression test: it fails again the
...
```
$ ruff check --select PLR0915 tests/test_shell_and_app.py
All checks passed!
```
