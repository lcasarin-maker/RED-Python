...
def validate_chunks(file_path) -> bool:
    ...
    try:
        ...
        _logger.warning("validate_chunks: cannot read %s: %s", file_path, e)
        return False
    ...
    if file_path.suffix == ".py":
        try:
            ...
            _logger.warning("validate_chunks: syntax error in %s: %s", file_path, e)
            return False
    elif file_path.suffix == ".json":
        try:
            json.loads(content)
        except Exception as e:
            _logger.warning("validate_chunks: invalid JSON in %s: %s", file_path, e)
            return False
