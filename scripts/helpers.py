...
def _extract_json_facts(session_content: str, facts: dict) -> bool:
    ...
    try:
        ...
        answers = (
            data.get("answers", {}) if isinstance(data.get("answers"), dict) else data
        )

        learning = (
            answers.get("q1_learning") or data.get("learning_1") or data.get("learning")
        )
        if learning:
            facts["key_learnings"].append(str(learning)[:120])

        violation = (
            answers.get("q2_violation")
            or data.get("violation")
        ...
        facts["decisions"] = (
            ...
            else ([decisions] if isinstance(decisions, str) and decisions else [])
        )

        next_agent = (
            answers.get("q3_next_agent")
            or data.get("next_agent_knows")
...
def compress_session_to_fact_summary(session_id: str, session_content: str) -> str:
    ...
    status_icon = (
        "✅"
        if any(tok in facts["status"] for tok in ("COMPLETO", "SUCCESS", "COMPLETED"))
        else "⚠️"
    )
...
def compress_memory_block(memory_content: str) -> dict:
    ...
    for session in sessions:
        try:
            ...
            compressed["fact_summaries"].append(
                f"[{session.get('id', 'unknown')}] (failed to compress: {e})"
            )
