...
def extract_rules_touched(session_content: str) -> set[int]:
    ...
    try:
        ...
        if json_match:
            return {int(r.strip()) for r in json_match.group(1).split(',') if r.strip().lstrip('-').isdigit()}
    except Exception as e:
        _logger.debug("extract_rules_touched: could not parse rules from session: %s", e)
    return set()
...
def merge_historial_3way(ours_path, theirs_path, base_path=None):
    ...
    base_content = Path(base_path).read_text(encoding='utf-8') if base_path else ours_content
    ...
    # Merge automatically
    merged_sessions = merge_non_conflicting(sessions_ours, sessions_theirs, sessions_base)
...
def git_merge_driver(ancestor_path, ours_path, theirs_path):
    ...
    try:
        ...
        print(f"[SEMANTIC MERGE] Auto-resolved: {result['auto_resolved']} | Conflicts: {result['conflicts']}")
