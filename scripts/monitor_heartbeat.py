...
class HeartbeatMonitor:
    def __init__(self, heartbeat_dir=None):
        ...
        if heartbeat_dir is None:
            self.heartbeat_dir = Path(
                r"D:\GoogleDrive\AI\.secrets\protocolo\heartbeats"
            )
        else:
    ...
    def ping_project(self, project_name, project_path):
        ...
        heartbeat = {
            ...
            "historial_last_seen": (
                last_heartbeat.get("historial_modified") if last_heartbeat else None
            ),
        }
        ...
        if last_heartbeat and historial_modified:
            ...
            if last_time:
                ...
                if delta > 10 and historial_modified == last_heartbeat.get(
                    "historial_modified"
                ):
    ...
    def run(self):
        ...
        if self.alerts:
            ...
            for alert in self.alerts:
                print(
                    f"   • [{alert['severity']}] {alert['project']}: {alert['message']}"
                )
        else:
