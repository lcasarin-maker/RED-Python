...
def candidate_files(root: Path) -> list[tuple[Path, bool]]:
    ...
    settings.local.json is intentionally excluded: it is a Claude Code session
    artifact populated at runtime by the tool (allowed Bash commands granted via
    the UI). Auditing it would produce false positives against legitimate tool
    invocations and is outside the project's control plane.
    """
    return [
