...
def bump(version: str, part: str) -> str:
    if part not in ("major", "minor", "patch"):
        raise ValueError(
            f"Invalid part '{part}'. Must be 'major', 'minor', or 'patch'."
        )
    ...
    for filepath, target, replacement in files_to_update:
        if filepath.exists():
            ...
            if target in content:
                ...
                print(
                    f"Propagated to {filepath.name} (exact match): {target} -> {replacement}"
                )
            else:
                ...
                if pattern.search(content):
                    ...
                    filepath.write_text(content, encoding="utf-8")
                    print(
                        f"Propagated to {filepath.name} (regex): v{old_version} -> v{new_version}"
                    )
...
def main():
    ...
    parser.add_argument(
        "--tag", action="store_true", help="Create a git tag after bump"
    )
    ...
    try:
        ...
        print(f"[ERROR] bump_version: {e}", flush=True)
